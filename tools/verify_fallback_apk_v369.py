...
apk=Path(sys.argv[1] if len(sys.argv)>1 else '/mnt/data/MarketRiskMonitor_GlobalPlus_v3.6.9_BUILD119_signed.apk')
...
with zipfile.ZipFile(apk) as z:
    ...
    manifest=z.read('AndroidManifest.xml'); html=z.read('assets/index.html'); excel_js=z.read('assets/excel_export_v362.js'); shortage_js=z.read('assets/global_shortage_v369.js'); dex=z.read('classes.dex'); res=z.read('resources.arsc')
    blob=apk.read_bytes()
    for i in z.infolist():
...
assert man['package'][0]=='com.marko.marketrisk.globalplus'; assert man['versionCode'][2]==119; assert man['versionName'][0]=='3.6.9'
...
print('PASS - embedded index.html, excel_export_v362.js and global_shortage_v369.js byte-identical to release source')
