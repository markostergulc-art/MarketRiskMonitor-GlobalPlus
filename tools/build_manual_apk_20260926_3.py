...
OUT=ROOT/'manual_apk_build_20260926_3'
...
P_VIEW_INT=(VIEW,(INT,)); P_BOOL0=(BOOL,()); P_BOOL_INT=(BOOL,(INT,)); P_BOOL_BOOL=(BOOL,(BOOL,))
...
methods=[
 (ACT,'<init>',P_VOID0),(ACT,'onCreate',P_VOID_BUNDLE),(ACT,'onBackPressed',P_VOID0),(ACT,'moveTaskToBack',P_BOOL_BOOL),(ACT,'setContentView',P_VOID_VIEW),(ACT,'findViewById',P_VIEW_INT),(ACT,'requestWindowFeature',P_BOOL_INT),
...
a.emit(const4(1,7)); a.emit(invoke(0x6e,method_idx[(ACT,'findViewById',P_VIEW_INT)],[2,1])); a.emit(moveresobj(0)); a.emit(checkcast(0,type_idx[WEBVIEW])); a.emit(invoke(0x6e,method_idx[(WEBVIEW,'canGoBack',P_BOOL0)],[0])); a.emit(moveres(1)); a.ifz(1,'super'); a.emit(invoke(0x6e,method_idx[(WEBVIEW,'goBack',P_VOID0)],[0])); a.emit(retvoid()); a.label('super'); a.emit(const4(1,1)); a.emit(invoke(0x6e,method_idx[(ACT,'moveTaskToBack',P_BOOL_BOOL)],[2,1])); a.emit(moveres(1)); a.emit(retvoid()); units=a.patch(); data+=code_item(3,1,2,units); align4(data)
...
sp=list(attr_ids)+['android','http://schemas.android.com/apk/res/android','manifest','uses-sdk','uses-permission','application','activity','intent-filter','action','category','package','com.marko.marketrisk.globalplus','2026.09.26.3','Market Risk Monitor Global+','android.permission.INTERNET','com.marko.marketrisk.globalplus.MainActivity','android.intent.action.MAIN','android.intent.category.LAUNCHER']
...
xml+=sel('manifest',[attr(None,'package','com.marko.marketrisk.globalplus'),attr(AURI,'versionCode',dtype=TYPE_INT_DEC,data_val=126),attr(AURI,'versionName','2026.09.26.3')])
...
# ---------------- assets + APK ----------------
html=(ROOT/'app/src/main/assets/index.html').read_bytes();excel_js=(ROOT/'app/src/main/assets/excel_export_v362.js').read_bytes();t212_js=(ROOT/'app/src/main/assets/t212_portfolio_v363.js').read_bytes();cr116_js=(ROOT/'app/src/main/assets/capital_rotation_contributors_v366.js').read_bytes();cr117_js=(ROOT/'app/src/main/assets/capital_rotation_ui_v367.js').read_bytes();shortage_js=(ROOT/'app/src/main/assets/global_shortage_v369.js').read_bytes();intel_js=(ROOT/'app/src/main/assets/intelligence_v39.js').read_bytes();alerts_js=(ROOT/'app/src/main/assets/smart_alerts_v39.js').read_bytes();economic_snapshot_js=(ROOT/'app/src/main/assets/economic_structure_snapshot_v126.js').read_bytes();economic_js=(ROOT/'app/src/main/assets/economic_structure_v126.js').read_bytes();economic_db=(ROOT/'app/src/main/assets/economic_structure_snapshot.db').read_bytes();market_details_js=(ROOT/'app/src/main/assets/market_details_v310.js').read_bytes();icon=(ROOT/'app/src/main/assets/app_icon.png').read_bytes();(OUT/'index.html').write_bytes(html);(OUT/'excel_export_v362.js').write_bytes(excel_js);(OUT/'t212_portfolio_v363.js').write_bytes(t212_js);(OUT/'capital_rotation_contributors_v366.js').write_bytes(cr116_js);(OUT/'capital_rotation_ui_v367.js').write_bytes(cr117_js);(OUT/'global_shortage_v369.js').write_bytes(shortage_js);(OUT/'intelligence_v39.js').write_bytes(intel_js);(OUT/'smart_alerts_v39.js').write_bytes(alerts_js);(OUT/'economic_structure_snapshot_v126.js').write_bytes(economic_snapshot_js);(OUT/'economic_structure_v126.js').write_bytes(economic_js);(OUT/'economic_structure_snapshot.db').write_bytes(economic_db);(OUT/'market_details_v310.js').write_bytes(market_details_js);(OUT/'app_icon.png').write_bytes(icon)
unsigned=OUT/'MarketRiskMonitor_GlobalPlus_2026.09.26.3_unsigned.apk'
with zipfile.ZipFile(unsigned,'w',compression=zipfile.ZIP_DEFLATED) as z:
    ...
    z.write(OUT/'excel_export_v362.js','assets/excel_export_v362.js',compress_type=zipfile.ZIP_DEFLATED)
    z.write(OUT/'t212_portfolio_v363.js','assets/t212_portfolio_v363.js',compress_type=zipfile.ZIP_DEFLATED)
    z.write(OUT/'capital_rotation_contributors_v366.js','assets/capital_rotation_contributors_v366.js',compress_type=zipfile.ZIP_DEFLATED)
    z.write(OUT/'capital_rotation_ui_v367.js','assets/capital_rotation_ui_v367.js',compress_type=zipfile.ZIP_DEFLATED)
    z.write(OUT/'global_shortage_v369.js','assets/global_shortage_v369.js',compress_type=zipfile.ZIP_DEFLATED)
    z.write(OUT/'intelligence_v39.js','assets/intelligence_v39.js',compress_type=zipfile.ZIP_DEFLATED)
    z.write(OUT/'smart_alerts_v39.js','assets/smart_alerts_v39.js',compress_type=zipfile.ZIP_DEFLATED)
    z.write(OUT/'economic_structure_snapshot_v126.js','assets/economic_structure_snapshot_v126.js',compress_type=zipfile.ZIP_DEFLATED)
    z.write(OUT/'economic_structure_v126.js','assets/economic_structure_v126.js',compress_type=zipfile.ZIP_DEFLATED)
    z.write(OUT/'economic_structure_snapshot.db','assets/economic_structure_snapshot.db',compress_type=zipfile.ZIP_DEFLATED)
    z.write(OUT/'market_details_v310.js','assets/market_details_v310.js',compress_type=zipfile.ZIP_DEFLATED)
...
# Produce a deterministic 4-byte-aligned APK for structural verification. Android's
# zipalign tool is not assumed to be installed in the remediation environment.
aligned=OUT/'MarketRiskMonitor_GlobalPlus_2026.09.26.3_unsigned_aligned.apk'
with zipfile.ZipFile(unsigned,'r') as zin, zipfile.ZipFile(aligned,'w') as zout:
    for src_info in zin.infolist():
        ...
        if zi.compress_type==zipfile.ZIP_STORED:
            ...
            if base%4:
                pad_data=(-base-4)%4
                zi.extra=struct.pack('<HH',0xffff,pad_data)+(b'\0'*pad_data)
        zout.writestr(zi,data)
print('aligned',aligned,aligned.stat().st_size)
