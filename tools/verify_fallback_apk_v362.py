...
apk=Path(sys.argv[1] if len(sys.argv)>1 else '/mnt/data/MarketRiskMonitor_GlobalPlus_v3.6.2_unsigned.apk')
...
with zipfile.ZipFile(apk) as z:
    ...
    manifest=z.read('AndroidManifest.xml'); html=z.read('assets/index.html'); excel_js=z.read('assets/excel_export_v362.js'); dex=z.read('classes.dex'); res=z.read('resources.arsc')
    blob=apk.read_bytes()
    for i in z.infolist():
...
assert man['package'][0]=='com.marko.marketrisk.globalplus'; assert man['versionCode'][2]==112; assert man['versionName'][0]=='3.6.2'
...
source_html=Path('app/src/main/assets/index.html').read_bytes(); source_excel_js=Path('app/src/main/assets/excel_export_v362.js').read_bytes(); assert html==source_html; assert excel_js==source_excel_js
...
print('PASS - embedded index.html and excel_export_v362.js byte-identical to release source')
