...
apk=Path(sys.argv[1] if len(sys.argv)>1 else '/mnt/data/MarketRiskMonitor_GlobalPlus_2026.09.26.2_BUILD125_signed.apk')
...
with zipfile.ZipFile(apk) as z:
    ...
    manifest=z.read('AndroidManifest.xml'); html=z.read('assets/index.html'); excel_js=z.read('assets/excel_export_v362.js'); shortage_js=z.read('assets/global_shortage_v369.js'); intel_js=z.read('assets/intelligence_v39.js'); alerts_js=z.read('assets/smart_alerts_v39.js'); economic_js=z.read('assets/economic_structure_v125.js'); market_details_js=z.read('assets/market_details_v310.js'); dex=z.read('classes.dex'); res=z.read('resources.arsc')
    blob=apk.read_bytes()
    for i in z.infolist():
...
assert man['package'][0]=='com.marko.marketrisk.globalplus'; assert man['versionCode'][2]==125; assert man['versionName'][0]=='2026.09.26.2'
...
source_html=Path('app/src/main/assets/index.html').read_bytes(); source_excel_js=Path('app/src/main/assets/excel_export_v362.js').read_bytes(); source_shortage_js=Path('app/src/main/assets/global_shortage_v369.js').read_bytes(); source_intel_js=Path('app/src/main/assets/intelligence_v39.js').read_bytes(); source_alerts_js=Path('app/src/main/assets/smart_alerts_v39.js').read_bytes(); source_economic_js=Path('app/src/main/assets/economic_structure_v125.js').read_bytes(); source_market_details_js=Path('app/src/main/assets/market_details_v310.js').read_bytes(); assert html==source_html; assert excel_js==source_excel_js; assert shortage_js==source_shortage_js; assert intel_js==source_intel_js; assert alerts_js==source_alerts_js; assert economic_js==source_economic_js; assert market_details_js==source_market_details_js
...
print('PASS - embedded index.html, economic_structure_v125.js, market_details_v310.js and existing release assets byte-identical to release source')
