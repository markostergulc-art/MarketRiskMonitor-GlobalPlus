...
const base50=fs.readFileSync('app_v35150_baseline.js','utf8');
...
// I: scheduler/loading observer and protected financial/fiscal functions unchanged from v50.
for(const n of ['loadPriorityForV44','pumpLoadSchedulerV44','queueLoadV44','scheduleDeferredV44','scheduleAroundPageV44','refreshActiveV44','marketMetrics','trendRisk','corrMatrix','correlationRegimeFromStats','correlationRegimeDiagnostic','buildMacroCycle','sp500HiddenWeakness','companyCoreRiskParts','companyContextRisk','fiscalFreshnessV44','fiscalPickReferenceV44','fiscalBandV43','fiscalScoreRowsV43']) ok(extract(src,n)===extract(base50,n),n+' byte-identical to v50');

// J: no additional network calls; fetch wrapper unchanged.
ok(extract(src,'fetchWithTimeout')===extract(base50,'fetchWithTimeout'),'network fetch wrapper unchanged; period label adds no request');
ok(html.includes('.market-spark-period-v46'),'compact mobile Markets-card period CSS present');
ok(/versionCode\s+84/.test(fs.readFileSync('app/build.gradle','utf8'))&&/versionName\s+'3.5.1.51'/.test(fs.readFileSync('app/build.gradle','utf8')),'v3.5.1.51 / versionCode 84 metadata');
ok(html.includes('v3.5.1_51'),'visible UI version is v3.5.1_51');
...
console.log(`RESULT - ${p}/${p+f} v3.5.1.51 Markets-card sparkline-period regression checks PASS`);if(f)process.exit(1);
