...
ok(src.includes("if(page==='markets')")&&src.includes('ensureFiscalV43(false)'),'fiscal data lazy-loads from Markets');
...
ok(src.includes("comparability=coreHarmonized?'HARMONIZED':eu?'MIXED_FALLBACK':'CENTRAL_GOVERNMENT_FALLBACK'"),'definition/comparability warning retained');
...
ok(html.includes('v3.5.1_43'),'visible version label updated');
console.log(`RESULT - ${pass}/${pass+fail} v3.5.1.43 fiscal checks PASS`);if(fail)process.exit(1);
