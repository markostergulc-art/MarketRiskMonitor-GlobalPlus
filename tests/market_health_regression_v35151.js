...
// Scenario B: one synthetic limited market cannot poison denominator.
let limited={code:'LIMITED',series:[]};
let b=aggregateEquityInternalsV42([...countries,limited],{...by,LIMITED:unavailableEquityInternalsV42(limited,'INSUFFICIENT_HISTORY')});
ok(b.score===a.score && b.validMarkets===36 && b.totalMarkets===37,'Scenario B: one limited market does not alter valid aggregate score');
ok(b.limitedMarkets.some(x=>x.code==='LIMITED'),'synthetic limited market is explicitly classified as ineligible for aggregate');
...
// v47 market-universe rule: Bosnia is removed, not represented as LIMITED.
ok(!src.includes("code:'BA'") && !src.includes("SASX-10") && src.includes("code:'HR'"),'v47 universe keeps Croatia and removes Bosnia runtime market config');
...
ok(html.includes('v3.5.1_51'),'user-visible version label updated to v3.5.1_51');
ok(src.includes("window.MRMV42Diagnostics"),'internal diagnostics remain available without normal UI clutter');
...
console.log(`RESULT - ${pass}/${pass+fail} v3.5.1.51 market-health/Overview regression checks PASS`);
