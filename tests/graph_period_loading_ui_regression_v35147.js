...
const base=fs.readFileSync('/mnt/data/mrm_v35145/app_v44_baseline.js','utf8');
...
const protectedFns=['globalRiskModel','globalContagion','marketMetrics','trendRisk','corrMatrix','correlationRegimeFromStats','correlationRegimeDiagnostic','buildMacroCycle','sp500HiddenWeakness','companyCoreRiskParts','companyContextRisk','fiscalFreshnessV44','fiscalPickReferenceV44','fiscalBandV43','fiscalScoreRowsV43'];
...
ok(/versionCode\s+80/.test(fs.readFileSync('app/build.gradle','utf8'))&&/versionName\s+'3\.5\.1\.47'/.test(fs.readFileSync('app/build.gradle','utf8')),'v3.5.1.47 / versionCode 80 source metadata');
ok(html.includes('v3.5.1_47'),'visible version label is v3.5.1_47');
...
console.log(`RESULT - ${p}/${p+f} v3.5.1.47 graph/loading UI regression checks PASS`);if(f)process.exit(1);
