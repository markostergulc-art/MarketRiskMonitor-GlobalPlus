...
ck('21 sheet specs declared', len(re.findall(r"name:'(?:0\d|1\d|20) [^']+'",js))==21)
...
ck('report version 3.6.2/112', "XLSX_VERSION_V362='3.6.2'" in js and 'XLSX_VERSION_CODE_V362=112' in js)
...
eval(fs.readFileSync(process.argv[2],'utf8')); (async()=>{let a=window.__MRM_XLSX_TEST__,s=a.sheetSpecsV362(); if(s.length!==21)throw new Error('sheets '+s.length);let b=a.xlsxBlobV362(s);fs.writeFileSync(process.argv[3],Buffer.from(await b.arrayBuffer()));})();
'''
    ...
    if ok:
        try:
            with zipfile.ZipFile(out) as z:
                ...
                ck('XLSX contains 21 worksheets',len(sheets)==21)
                xmlok=True
                for n in z.namelist():
            ...
            ck('XLSX ZIP integrity',False); ck('XLSX contains 21 worksheets',False); ck('OOXML parts are well-formed XML',False)
