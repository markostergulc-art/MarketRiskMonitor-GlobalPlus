...
ok(/versionCode\s+90\b/.test(gradle),'versionCode 90');
ok(/versionName\s+['"]3\.5\.1\.57['"]/.test(gradle),'versionName 3.5.1.57');
ok(html.includes('v3.5.1_57'),'visible header v3.5.1_57');
ok(/VERSION_NAME="3\.5\.1\.57"/.test(release)&&/VERSION_CODE="90"/.test(release),'release metadata v3.5.1.57 / 90');
ok(java.includes('MarketRiskMonitor/3.5.1.57'),'native user-agent version 3.5.1.57');
ok(src.includes("const EXPORT_V47_VERSION='3.5.1.57',EXPORT_V47_CODE=90"),'export metadata v3.5.1.57 / 90');
...
console.log(`RESULT - ${p}/${p+f} Phase 6 compatibility checks under Phase 7 ${f?'FAIL':'PASS'}`);if(f)process.exit(1);
