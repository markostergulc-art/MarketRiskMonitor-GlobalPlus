...
ok(src.includes("const EXPORT_V47_VERSION='3.5.1.47',EXPORT_V47_CODE=80"),'export package is versioned v3.5.1.47 / 80');
...
ok(/versionCode\s+80/.test(fs.readFileSync('app/build.gradle','utf8'))&&/versionName\s+'3\.5\.1\.47'/.test(fs.readFileSync('app/build.gradle','utf8')),'release metadata is v3.5.1.47 / versionCode 80');
ok(html.includes('v3.5.1_47'),'visible UI version is v3.5.1_47');
...
(async()=>{let z=ctx.zipStoreV47([{name:'a.txt',data:'alpha'},{name:'b.json',data:'{\"ok\":true}'}]);let buf=Buffer.from(await z.arrayBuffer());fs.writeFileSync('/mnt/data/mrm_v35147/test_export_v47.zip',buf);ok(buf.readUInt32LE(0)===0x04034b50&&buf.includes(Buffer.from('a.txt')),'ZIP writer produces standard ZIP local headers and filenames');
...
console.log(`RESULT - ${p}/${p+f} v3.5.1.47 export/universe checks PASS`);if(f)process.exit(1)})().catch(e=>{console.error(e);process.exit(1)});
