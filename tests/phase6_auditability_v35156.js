...
const base=fs.readFileSync(path.join(root,'app_v35155_baseline.js'),'utf8');
...
ok(/versionCode\s+89\b/.test(gradle),'versionCode 89');
ok(/versionName\s+['"]3\.5\.1\.56['"]/.test(gradle),'versionName 3.5.1.56');
ok(html.includes('v3.5.1_56'),'visible header v3.5.1_56');
ok(/VERSION_NAME="3\.5\.1\.56"/.test(release)&&/VERSION_CODE="89"/.test(release),'release metadata v3.5.1.56 / 89');
ok(java.includes('MarketRiskMonitor/3.5.1.56'),'native user-agent version 3.5.1.56');
ok(src.includes("const EXPORT_V47_VERSION='3.5.1.56',EXPORT_V47_CODE=89"),'export metadata v3.5.1.56 / 89');
...
console.log(`RESULT - ${p}/${p+f} Phase 6 auditability checks ${f?'FAIL':'PASS'}`);if(f)process.exit(1);
