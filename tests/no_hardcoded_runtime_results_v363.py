...
portfolio=(ROOT/'app/src/main/assets/t212_portfolio_v363.js').read_text()
...
ck('export metadata versionCode is build 113', "EXPORT_V47_CODE=113" in app)


ck('T212 module contains no hardcoded company status map', not re.search(r"(?:AAPL|MSFT|NVDA|TSLA|AMZN|GOOGL|META)\s*[:=]\s*['\"](?:KEEP|WATCH|SELL|REDUCE)", portfolio, re.I))
ck('T212 module contains no embedded live market prices', not re.search(r"(?:AAPL|MSFT|NVDA|TSLA).{0,80}(?:price|currentPrice)\s*[:=]\s*\d+(?:\.\d+)?", portfolio, re.I))
ck('T212 module explicitly normalizes missing metrics instead of zero filling', 'weightedScore' in portfolio and 'Number.isFinite(v)' in portfolio)
