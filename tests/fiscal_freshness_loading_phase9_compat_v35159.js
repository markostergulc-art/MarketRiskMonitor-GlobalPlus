...
ok(src.includes("r.rankingEligible=Number.isFinite(r.score)&&!(r.staleMetrics||[]).length&&r.comparability!=='COMPARABILITY LIMITED'"),'stale/incomparable rows are excluded from current rankings');
...
ok(html.includes('id="loadPriorityStatus"')&&html.includes('v3.5.1_59'),'loading-priority status UI and v44 visible version present');
console.log(`RESULT - ${p}/${p+f} v3.5.1.59 Phase 8 compatibility freshness/loading checks PASS`);if(f)process.exit(1);
